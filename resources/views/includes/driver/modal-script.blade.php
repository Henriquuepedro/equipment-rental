<script>
        ...
        submitHandler: function(form) {
            ...
            $.ajax({
                ...
                success: response => {
                    ...
                    checkLabelAnimate();
                    loadDrivers(response.driver_id);
                }, error: e => {
                    getForm.find('button[type="submit"]').attr('disabled', false);
                    let arrErrors = [];
    ...
    const loadDrivers = (driver_id = null) => {

        $('.driver-load [name="driver"]').empty().append('<option>Carregando ...</option>');
        ...
        $.ajax({
            ...
            success: response => {
                ...
                $('.driver-load [name="driver"]').empty().append('<option>Selecione ...</option>');
                $.each(response.data, function( index, value ) {
                    selected = value.id === driver_id_selected ? 'selected' : '';
                    $('.driver-load [name="driver"]').append(`<option value='${value.id}' ${selected}>${value.name}</option>`);
                });
