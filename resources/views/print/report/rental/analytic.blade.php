                    ...
                    <table class="table equipment">
                        @foreach($rental as $key => $equipment)
                            <tr style="width: 33%" class="@if(count($rental) != 1 && $key == 0) eq-line-one @elseif(count($rental) == 1) eq-line-no-border @else eq-line @endif">
                                ...
                                <td class="text-right" style="width: 30%">{{ dateInternationalToDateBrazil($equipment['actual_delivery_date'], DATETIME_BRAZIL_NO_SECONDS) ?? 'Não entregue' }}</td>
                                <td class="text-right" style="width: 30%">{{ dateInternationalToDateBrazil($equipment['actual_withdrawal_date'], DATETIME_BRAZIL_NO_SECONDS) ?? 'Não retirado' }}</td>
                            </tr>
                        @endforeach
                    </table>
                </td>
