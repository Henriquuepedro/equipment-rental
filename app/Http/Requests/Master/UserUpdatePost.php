...
class UserUpdatePost extends FormRequest
{
        ...
        return [
            ...
            'email'       => [
                ...
                function ($attribute, $value, $fail) {
                    $exists = DB::table('users')->where('email', $value)
                        ->whereNotIn('id', [$this->route('id')])
                        ->count();
                    if ($exists) {
                        $fail('E-mail informado já está em uso.');
                    }
                }
            ],
    ...
    public function messages(): array
    {
        return [
            ...
            'type_user'         => 'Tipo de usuário deve ser um entre: User, Admin e Admin-Master.'
        ];
    }
}
