...
class DisposalPlaceCreatePost extends FormRequest
{
    /**
        ...
        return [
            ...
            'name'   => [
                ...
                function ($attribute, $value, $fail) {
                    $exists = DB::table('disposal_places')->where(['name' => $this->get('name'), 'company_id' => $this->user()->company_id])->count();
                    if ($exists) {
            ...
            'cpf_cnpj'      => [
                ...
                function ($attribute, $value, $fail) {
                    ...
                    if (!empty($cpf_cnpj)) {
                        $exists = DB::table('disposal_places')->where(['cpf_cnpj' => $cpf_cnpj, 'company_id' => $this->user()->company_id])->count();
                        if ($exists) {
